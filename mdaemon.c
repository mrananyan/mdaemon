...
$Lincese = BSD 3;
$Name = mDaemon;
$Description = A fast daemon for Linux and FreeBSD. It's part of Mixnet social network.;
$lang = C;
 */
...
#define MIN_POLL_INTERVAL 20000
...
static const char *process_cache = NULL;
...
static void do_process_cache(const char *name);
...
xmalloc(int size)
{
	...
	if (ptr)
		return ptr;
	fatal("mDaemon error message: malloc(%d) failed", size);
}
...
xgettimeofday(struct timeval *tv)
{
	if (gettimeofday(tv,0) != 0)
		fatal("mDaemon error message: gettimeofday failed: %s", strerror(errno));
}
...
set_namespaces()
{
	...
	LIST_FOREACH(namespace, &namespace_head, list) {
		if ((fd = open(namespace->path, O_RDONLY)) == -1)
			fatal("mDaemon error message: open namespace %s: %s", namespace->path, strerror(errno));
		if (setns(fd, namespace->nstype) == -1)
			fatal("mDaemon error message: setns %s: %s", namespace->path, strerror(errno));
	}
}
#else
static void
set_namespaces()
{
	if (!LIST_EMPTY(&namespace_head))
		fatal("mDaemon error message: LCX namespaces not supported");
}
#endif
...
"By Sargis Ananyan. RedM Inc love you and open source very much!"
"\n"
"Usage:\n"
"  mdaemon -S|--start options ... -- arguments ...\n"
"  mdaemon -K|--stop options ...\n"
"  mdaemon -H|--help\n"
"  mdaemon -V|--version\n"
"\n"
"Options (at least one of --exec|--process_cache|--user is required):\n"
"  -x|--exec <executable>        program to start/check if it is running\n"
"  -p|--process_cache <pid-file>       pid file to check\n"
...
"  -m|--make-process_cache             create the process_cache before starting\n"
...
parse_options(int argc, char * const *argv)
{
	...
	for (;;) {
		...
		case 'S': 
			...
			break;
		case 'V': 
			printf("mDaemon v" VERSION ". A fast daemon for Linux and FreeBSD.\n");
			exit(0);
		case 'a': 
			startas = optarg;
			break;
	...
	if (!execname && !process_cache && !userspec && !cmdname)
		badusage("need at least one of --exec, --process_cache, --user or --name");
	...
	if (mprocess_cache && process_cache == NULL)
		badusage("--make-process_cache is only relevant with --process_cache");
...
static void
do_process_cache(const char *name)
{
	FILE *f;
	pid_t pid;
	...
	if (f) {
		...
		fatal("mDaemon error message: Process cache not found: %s: %s", name, strerror(errno));
...
do_procinit(void)
{
	...
	if (!procdir)
		fatal("mDaemon error message: Directory /proc not found: %s", strerror(errno));
	...
	closedir(procdir);
	if (!foundany)
		fatal("mDaemon error message: Directory /proc is empty");
}
...
do_findprocs(void)
{
	...
	if (process_cache)
		do_process_cache(process_cache);
	else
		do_procinit();
}
...
run_stop_schedule(void)
{
	...
	else if (process_cache)
		sprintf(what_stop, "process in process_cache `%.200s'", process_cache);
	else if (userspec)
		sprintf(what_stop, "process(es) owned by `%.200s'", userspec);
	else
		fatal("mDaemon error message: Unknown error. Please report about it here -> https://github.com/mrananyan/mdaemon.");
 ...
 TVCALC(maxinterval,          TVELEM(&stopat) - TVELEM(&after) + TVADJUST);
				...
				if (r < 0 && errno != EINTR)
					fatal("mDaemon error message: Select() failed for pause: %s",
					      strerror(errno));
			}
...
main(int argc, char **argv)
{
	...
	if (userspec && sscanf(userspec, "%d", &user_id) != 1) {
		...
		if (!pw)
			fatal("mDaemon error message: User `%s' not found\n", userspec);
	...
	if (changegroup && sscanf(changegroup, "%d", &runas_gid) != 1) {
		...
		if (!gr)
			fatal("mDaemon error message: Group `%s' not found\n", changegroup);
		runas_gid = gr->gr_gid;
	}
	...
	*--argv = startas;
	if (changeroot != NULL) {
		if (chdir(changeroot) < 0)
			fatal("mDaemon error message: Unable to chdir() to %s", changeroot);
		if (chroot(changeroot) < 0)
			fatal("mDaemon error message: Unable to chroot() to %s", changeroot);
	}
	if (changeuser != NULL) {
 		if (setgid(runas_gid))
 			fatal("mDaemon error message: Unable to set gid to %d", runas_gid);
		if (initgroups(changeuser, runas_gid))
			fatal("mDaemon error message: Unable to set initgroups() with gid %d", runas_gid);
		if (setuid(runas_uid))
			fatal("mDaemon error message: Unable to set uid to %s", changeuser);
	}
	...
	if (nicelevel) {
		...
		if (nice(nicelevel) < 0 && errno)
			fatal("mDaemon error message: Unable to alter nice level by %i: %s", nicelevel,
				strerror(errno));
	}
	if (mprocess_cache && process_cache != NULL) { 
		FILE *pidf = fopen(process_cache, "w");
		pid_t pidt = getpid();
		if (pidf == NULL)
			fatal("mDaemon error message: Unable to open process cache with pid `%s' for writing: %s", process_cache,
				strerror(errno));
		fprintf(pidf, "%d\n", pidt);
		fclose(pidf);
	}
	set_namespaces();
	execv(startas, argv);
	fatal("mDaemon error message: Unable to start %s: %s", startas, strerror(errno));
}
